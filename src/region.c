...
void initializeRegionArray(struct region regionArray[],
		const struct spec_region3D subvol_spec[], const short NUM_REGIONS,
		const unsigned short NUM_MOL_TYPES, const double SUBVOL_BASE_SIZE,
		double DIFF_COEF[NUM_REGIONS][NUM_MOL_TYPES],
		const unsigned short MAX_RXNS, const struct chem_rxn_struct * chem_rxn) {
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		...
		for (j = i + 1; j < NUM_REGIONS; j++) {
			...
			if (!strcmp(subvol_spec[i].label, subvol_spec[j].label)) {
				fprintf(stderr,
						"ERROR: Regions %u and %u have the same label \"%s\".\n",
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		...
		if (subvol_spec[i].shape == RECTANGULAR_BOX
			...
			regionArray[i].actualSubSize = subvol_spec[i].sizeRect
					* SUBVOL_BASE_SIZE;
			...
			if (subvol_spec[i].shape == RECTANGLE) { // Determine which plane the rectangle is in
				regionArray[i].dimension = DIM_2D;
				if (regionArray[i].actualSubSize * subvol_spec[i].numX == 0) {
					regionArray[i].plane = PLANE_YZ;
				} else if (regionArray[i].actualSubSize * subvol_spec[i].numY
						== 0) {
					regionArray[i].plane = PLANE_XZ;
				} else if (regionArray[i].actualSubSize * subvol_spec[i].numZ
						== 0) {
					regionArray[i].plane = PLANE_XY;
				} else { // We shouldn't have gotten here
					fprintf(stderr,
							"ERROR: Region %u (Label: \"%s\") is not defined properly as a Rectangle.\n",
							i, subvol_spec[i].label);
					fprintf(stderr,
							"There must be 0 subvolumes along one dimension.\n");
					bFail = true;
				}
			} else { // There is no single plane
			...
			regionArray[i].dimension = DIM_3D;
			regionArray[i].plane = PLANE_3D;
		} else if (subvol_spec[i].shape == CYLINDER) { //TODO changed, finish and test
			...
			regionArray[i].boundary[5] = SUBVOL_BASE_SIZE
					* (subvol_spec[i].numX + subvol_spec[i].numY
							+ subvol_spec[i].numZ);
			regionArray[i].dimension = DIM_3D;
			regionArray[i].plane = PLANE_3D;
		}
		...
		if (!regionArray[i].spec.bMicro) {
			...
			if (regionArray[i].diffRate == NULL) {
				fprintf(stderr,
						"ERROR: Memory allocation for region %u (label: \"%s\") diffusion coefficients.\n",
						i, subvol_spec[i].label);
				exit(EXIT_FAILURE);
			}
			for (curMolType = 0; curMolType < NUM_MOL_TYPES; curMolType++) {
				h_i = regionArray[i].actualSubSize;
				regionArray[i].diffRate[curMolType] = DIFF_COEF[i][curMolType]
						/ h_i / h_i;
			}
		}
	}
	...
	if (bFail) { // There was at least one error in the region configuration
		exit(EXIT_FAILURE);
	}
	...
	initializeRegionChemRxn(NUM_REGIONS, regionArray, NUM_MOL_TYPES, MAX_RXNS,
			chem_rxn, DIFF_COEF);
}
...
void initializeRegionSubNeighbor(struct region regionArray[],
		const struct spec_region3D subvol_spec[], const short NUM_REGIONS,
		const unsigned short NUM_MOL_TYPES, const double SUBVOL_BASE_SIZE,
		const double boundAdjError, struct subvolume3D * subvolArray,
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		...
		regionArray[i].boundSubNumFace = malloc(
				NUM_REGIONS * sizeof(unsigned short *));
		regionArray[i].boundSubCoor = malloc(
				NUM_REGIONS * sizeof(regionArray[i].boundSubCoor));
		regionArray[i].boundVirtualNeighCoor = malloc(
				NUM_REGIONS * sizeof(regionArray[i].boundVirtualNeighCoor));
		regionArray[i].bNeedUpdate = malloc(
				NUM_REGIONS * sizeof(regionArray[i].bNeedUpdate));
		regionArray[i].numMolFromMicro = malloc(
				NUM_REGIONS * sizeof(regionArray[i].numMolFromMicro));
		if (regionArray[i].neighID == NULL
			...
			fprintf(stderr,
					"ERROR: Memory allocation for region %u (label: \"%s\") neighbor parameters.\n",
					i, subvol_spec[i].label);
			exit(EXIT_FAILURE);
		}

		for (j = 0; j < NUM_REGIONS; j++) { // Scan boundary subvolumes to find those bordering this region
			...
			for (curID = regionArray[i].firstID;
					curID < (regionArray[i].firstID + regionArray[i].numSub);
					curID++) { // For each subvolume in current region
				...
				if (bSubFaceRegion(regionArray, j, curSubBound, boundAdjError,
						&numDir, dirArray)) {
			...
			regionArray[i].neighID[j] = malloc(
					regionArray[i].numSubRegionNeigh[j] * sizeof(uint32_t));
			regionArray[i].boundSubNumFace[j] = malloc(
					regionArray[i].numSubRegionNeigh[j]
							* sizeof(unsigned short));
			regionArray[i].boundSubCoor[j] = malloc(
					regionArray[i].numSubRegionNeigh[j] * sizeof(double[3]));
			regionArray[i].boundVirtualNeighCoor[j] = malloc(
					regionArray[i].numSubRegionNeigh[j]
							* sizeof(regionArray[i].boundVirtualNeighCoor[j]));
			regionArray[i].bNeedUpdate[j] = malloc(
					regionArray[i].numSubRegionNeigh[j] * sizeof(bool));
			regionArray[i].numMolFromMicro[j] = malloc(
					regionArray[i].numSubRegionNeigh[j]
			...
			for (curBoundID = 0;
					curBoundID < regionArray[i].numSubRegionNeigh[j];
					curBoundID++) {
				regionArray[i].bNeedUpdate[j][curBoundID] = false;
				regionArray[i].numMolFromMicro[j][curBoundID] = malloc(
						NUM_MOL_TYPES * sizeof(uint64_t));
				...
				for (curMolType = 0; curMolType < NUM_MOL_TYPES; curMolType++) {
					regionArray[i].numMolFromMicro[j][curBoundID][curMolType] =
							0ULL;
				}
			}
			...
			for (curID = regionArray[i].firstID;
				...
				if (bSubFaceRegion(regionArray, j, curSubBound, boundAdjError,
					...
					regionArray[i].boundVirtualNeighCoor[j][curBoundID] =
							malloc(
									regionArray[i].boundSubNumFace[j][curBoundID]
											* sizeof(double[3]));
					if (regionArray[i].boundVirtualNeighCoor[j][curBoundID]
							== NULL) {
						fprintf(stderr,
								"ERROR: Memory allocation for region %u (label: \"%s\")'s neighbor parameters with region %u (label: \"%s\").\n",
								i, subvol_spec[i].label, j,
								subvol_spec[j].label);
						exit(EXIT_FAILURE);
					}
					...
					for (curDir = 0;
						...
						default:
							fprintf(stderr,
									"ERROR: Invalid neighbor direction determined for neighboring regions %u and %u.\n",
									i, j);
						}
					}
...
void delete_boundary_region_(const short NUM_REGIONS,
		const unsigned short NUM_MOL_TYPES, struct region regionArray[]) {
	short i, j; // Current region
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		if (regionArray[i].numChildren > 0) {
			if (regionArray[i].childrenID != NULL)
				free(regionArray[i].childrenID);
			if (regionArray[i].childrenCoor != NULL)
				free(regionArray[i].childrenCoor);
		}

		if (regionArray[i].regionNeighDir != NULL)
			free(regionArray[i].regionNeighDir);

		if (regionArray[i].spec.bMicro) {
			if (regionArray[i].regionNeighID != NULL)
				free(regionArray[i].regionNeighID);
		...
		if (!regionArray[i].spec.bMicro && NUM_REGIONS > 1) {
			for (j = 0; j < NUM_REGIONS; j++) {
				...
				if (regionArray[i].neighID[j] != NULL)
					free(regionArray[i].neighID[j]);
				if (regionArray[i].boundSubNumFace[j] != NULL)
					free(regionArray[i].boundSubNumFace[j]);
				...
				if (regionArray[i].bNeedUpdate[j] != NULL)
					free(regionArray[i].bNeedUpdate[j]);
			}
			if (regionArray[i].neighID != NULL)
				free(regionArray[i].neighID);
			if (regionArray[i].boundSubNumFace != NULL)
				free(regionArray[i].boundSubNumFace);
			if (regionArray[i].boundSubCoor != NULL)
				free(regionArray[i].boundSubCoor);
			if (regionArray[i].boundVirtualNeighCoor != NULL)
				free(regionArray[i].boundVirtualNeighCoor);
			if (regionArray[i].bNeedUpdate != NULL)
				free(regionArray[i].bNeedUpdate);
			if (regionArray[i].numMolFromMicro != NULL)
				free(regionArray[i].numMolFromMicro);
		}

		if (regionArray[i].numSubRegionNeigh != NULL)
			free(regionArray[i].numSubRegionNeigh);
		if (regionArray[i].isRegionNeigh != NULL)
			free(regionArray[i].isRegionNeigh);
...
double findRegionVolume(const struct region regionArray[],
		const short curRegion, bool bOuter) {
	...
	case REGION_NORMAL:
		...
		volume = boundaryVolume(regionArray[curRegion].spec.shape,
	...
	case REGION_SURFACE_3D:
		...
		if (regionArray[curRegion].plane == PLANE_3D)
			volume = boundarySurfaceArea(regionArray[curRegion].spec.shape,
					regionArray[curRegion].boundary);
		else
			volume = boundaryVolume(regionArray[curRegion].spec.shape,
					regionArray[curRegion].boundary);
	...
	default:
		...
		fprintf(stderr,
				"ERROR: Region %u (label: \"%s\") has unknown type %d.\n",
				curRegion, regionArray[curRegion].spec.label,
				regionArray[curRegion].spec.type);
	}

	if (!bOuter) {
		// Subtract outer volumes of children
		for (curChild = 0; curChild < regionArray[curRegion].numChildren;
				curChild++) {
...
void findRegionTouch(const short NUM_REGIONS,
		const struct spec_region3D subvol_spec[], struct region regionArray[],
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		for (j = 0; j < NUM_REGIONS; j++) {
			if (i == j) { // Region is itself and not its neighbor
				continue;
			}
			...
			if (regionArray[i].spec.shape == SPHERE
					|| regionArray[j].spec.shape == SPHERE) { // Spherical regions can only touch parent/child regions
				continue;
			}

			if (regionArray[i].plane != regionArray[j].plane) { // Regions can only touch if one is normal 3D and other is 3D surface
				if (!(regionArray[i].plane == PLANE_3D
			...
			if (bBoundaryAdjacent(regionArray[i].spec.shape,
					regionArray[i].boundary, regionArray[j].spec.shape,
					regionArray[j].boundary, error, &direction)) { // Regions share a geometric face
																   // Check for surfaces
				...
				regionArray[i].boundaryRegionMicro[direction] =
						regionArray[j].spec.bMicro;
				regionArray[i].isRegionNeigh[j] = true;
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		for (j = i + 1; j < NUM_REGIONS; j++) {
			if (regionArray[i].isRegionNeigh[j]
					&& (regionArray[i].spec.type != REGION_NORMAL
							|| regionArray[j].spec.type != REGION_NORMAL)) { // Regions are neighbors and at least one of them is a surface
				if (regionArray[i].spec.bMicro != regionArray[j].spec.bMicro) {
					fprintf(stderr,
							"ERROR: Regions %u and %u (labels: \"%s\" and \"%s\") are neighbors in different regimes but at least one of them is a surface.\n",
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		...
		regionArray[i].regionNeighID = malloc(
				regionArray[i].numRegionNeigh * sizeof(short));
		if (regionArray[i].regionNeighID == NULL) {
			fprintf(stderr,
		...
		for (j = 0; j < NUM_REGIONS; j++) {
			...
			if (regionArray[i].isRegionNeigh[j]) { // Region i is microscopic and region j is a neighbor
				...
				regionArray[i].boundRegionFaceCoor[j] = malloc(
						regionArray[i].numRegionNeighFace[j]
								* sizeof(double[6]));
				...
				case PARENT:
					...
					for (k = 0; k < regionArray[i].numRegionNeighFace[j]; k++) {
						recordFace(regionArray[i].spec.shape,
								regionArray[i].boundary, k,
								regionArray[i].boundRegionFaceCoor[j][k]);
				...
				case CHILD:
					...
					// simulation
					for (k = 0; k < regionArray[i].numRegionNeighFace[j]; k++) {
						recordFace(regionArray[j].spec.shape,
								regionArray[j].boundary, k,
								regionArray[i].boundRegionFaceCoor[j][k]);
						regionArray[i].regionNeighFaceDir[j][k] = k;
					}
					break;
...
uint32_t findSubInBoundaryList(const short curRegion, const short destRegion,
		const struct region regionArray[], uint32_t curSub) {
	uint32_t neighID = 0;
...
unsigned short findNearestValidRegion(const double point[],
		const unsigned short sourceRegion, bool * bInRegion,
		const unsigned short NUM_REGIONS, const struct region regionArray[]) {
	...
	*bInRegion = false;
	for (curRegion = 0; curRegion < NUM_REGIONS; curRegion++) {
		...
		if ((regionArray[sourceRegion].isRegionNeigh[curRegion]
				|| curRegion == sourceRegion) && curDist < minDist) {
			minDist = curDist;
			minRegion = curRegion;
		}
	}
...
uint32_t findNearestSub(const short curRegion,
		const struct region regionArray[], const short neighRegion, double x,
		double y, double z) {
	...
	for (curSub = 0; curSub < numSub; curSub++) {
		curDistSq =
				(x - regionArray[curRegion].boundSubCoor[neighRegion][curSub][0])
						* (x
								- regionArray[curRegion].boundSubCoor[neighRegion][curSub][0]);
		...
		curDistSq +=
				(y - regionArray[curRegion].boundSubCoor[neighRegion][curSub][1])
						* (y
								- regionArray[curRegion].boundSubCoor[neighRegion][curSub][1]);
		if (curDistSq > minDistSq)
		...
		curDistSq +=
				(z - regionArray[curRegion].boundSubCoor[neighRegion][curSub][2])
						* (z
								- regionArray[curRegion].boundSubCoor[neighRegion][curSub][2]);

		if (curDistSq < minDistSq) { // This subvolume is the closest so far
			minSub = curSub;
			minDistSq = curDistSq;
		}
	}
...
void findChildCoordinates(const short parentRegion, const short childRegion,
		const short curChild, const struct region regionArray[]) {
	regionArray[parentRegion].childrenCoor[curChild][0] = (uint32_t) round(
			(regionArray[childRegion].boundary[0]
					- regionArray[parentRegion].boundary[0])
					/ regionArray[parentRegion].actualSubSize);
	regionArray[parentRegion].childrenCoor[curChild][1] = (uint32_t) round(
			(regionArray[childRegion].boundary[1]
					- regionArray[parentRegion].boundary[0])
					/ regionArray[parentRegion].actualSubSize - 1);
	regionArray[parentRegion].childrenCoor[curChild][2] = (uint32_t) round(
			(regionArray[childRegion].boundary[2]
					- regionArray[parentRegion].boundary[2])
					/ regionArray[parentRegion].actualSubSize);
	regionArray[parentRegion].childrenCoor[curChild][3] = (uint32_t) round(
			(regionArray[childRegion].boundary[3]
					- regionArray[parentRegion].boundary[2])
					/ regionArray[parentRegion].actualSubSize - 1);
	regionArray[parentRegion].childrenCoor[curChild][4] = (uint32_t) round(
			(regionArray[childRegion].boundary[4]
					- regionArray[parentRegion].boundary[4])
					/ regionArray[parentRegion].actualSubSize);
	regionArray[parentRegion].childrenCoor[curChild][5] = (uint32_t) round(
			(regionArray[childRegion].boundary[5]
					- regionArray[parentRegion].boundary[4])
					/ regionArray[parentRegion].actualSubSize - 1);
}
...
double intersectRegionVolume(const short curRegion,
		const struct region regionArray[], const int boundary2Type,
	...
	bool bArea = regionArray[curRegion].effectiveDim
			!= regionArray[curRegion].dimension;

	if (bArea
			&& bBoundarySurround(boundary2Type, boundary2,
					regionArray[curRegion].spec.shape,
					regionArray[curRegion].boundary, 0.)
			&& !bBoundarySurround(regionArray[curRegion].spec.shape,
					regionArray[curRegion].boundary, boundary2Type, boundary2,
					0.)) { // Boundary is fully inside region such that there is no intersection with region boundary
		return 0.;
	}
	...
	if (intersectType == UNDEFINED_SHAPE) {
		...
		fprintf(stderr,
				"ERROR: An intersection with region %u (label: \"%s\") is invalid.\nShapes are %s and %s",
				curRegion, regionArray[curRegion].spec.label,
				boundaryString(regionArray[curRegion].spec.shape),
	...
	for (curChild = 0; curChild < regionArray[curRegion].numChildren;
		...
		intersectType = intersectBoundary(regionArray[childRegion].spec.shape,
				regionArray[childRegion].boundary, boundary2Type, boundary2,
				interBoundary);
		...
		if (regionArray[childRegion].plane != PLANE_3D
				&& intersectType == RECTANGULAR_BOX) { // We still want to measure area of 2D region within 3D boundary
...
bool bLineHitRegion(const double p1[3], const double L[3], const double length,
		const short startRegion, const short endRegion,
		const struct region regionArray[], short * planeID, double * d,
	...
	if (regionArray[startRegion].parentID == endRegion) { // Boundary will be shape of the child
		boundary1Type = regionArray[startRegion].spec.shape;
		bInside = true;
	} else { // Boundary will be shape of the destination region
	...
	case RECTANGULAR_BOX:
		for (curPlane = 0;
				curPlane
						< regionArray[startRegion].numRegionNeighFace[endRegion];
				curPlane++) {
			if (bLineHitInfinitePlane(p1, L, length, RECTANGULAR_BOX,
					...
					&& bPointOnFace(intersectPoint, RECTANGULAR_BOX,
							regionArray[startRegion].boundRegionFaceCoor[endRegion][curPlane],
							regionArray[startRegion].regionNeighFaceDir[endRegion][curPlane])
					&& *d < minDist) { // Line does intersect this face at a valid distance and it is closest
				bIntersect = true;
				minDist = *d;
				*planeID =
						regionArray[startRegion].regionNeighFaceDir[endRegion][curPlane];
	...
	case SPHERE:
		return bLineHitInfinitePlane(p1, L, length, SPHERE,
				...
				regionArray[startRegion].regionNeighFaceDir[endRegion][0],
				bInside, d, intersectPoint);
	default:
		fprintf(stderr,
				"ERROR: Boundary type %d invalid for boundary intersection between regions %u and %u.\n",
...
bool bPointInRegionNotChild(const short curRegion,
		const struct region regionArray[], const double point[3]) {
	short curChild;

	if (bPointInBoundary(point, regionArray[curRegion].spec.shape,
			regionArray[curRegion].boundary)) { // Point is within the region's outer boundary
		for (curChild = 0; curChild < regionArray[curRegion].numChildren;
				curChild++) {
			if (bPointInBoundary(point,
					regionArray[regionArray[curRegion].childrenID[curChild]].spec.shape,
...
bool bPointInRegionOrChild(const short curRegion,
		const struct region regionArray[], const double point[3],
		short * actualRegion, bool bSurfaceOnly) {
	short curChild;
		...
		for (curChild = 0; curChild < regionArray[curRegion].numChildren;
			...
			if (bSurfaceOnly
					&& regionArray[regionArray[curRegion].childrenID[curChild]].spec.type
							== REGION_NORMAL)
				continue;
			if (bPointInRegionOrChild(
					regionArray[curRegion].childrenID[curChild], regionArray,
					point, actualRegion, bSurfaceOnly)) {
				return true;
			}
		}
...
bool bSharedBoundary(const short startRegion, const short endRegion,
		const struct region regionArray[], const short faceID) {
	...
	case RECTANGULAR_BOX:
		return fabs(
				regionArray[startRegion].boundary[faceID]
...
bool bChildRegionNotFlush(const short parentRegion, const short childRegion,
		const struct region regionArray[], const double boundAdjError) {
	return fabs(
			(regionArray[childRegion].boundary[0]
					- regionArray[parentRegion].boundary[0])
					/ regionArray[parentRegion].actualSubSize
			...
			|| fabs(
					(regionArray[childRegion].boundary[1]
							- regionArray[parentRegion].boundary[1])
							/ regionArray[parentRegion].actualSubSize
							- round(
									(regionArray[childRegion].boundary[1]
											- regionArray[parentRegion].boundary[1])
			...
			|| fabs(
					(regionArray[childRegion].boundary[3]
							- regionArray[parentRegion].boundary[3])
							/ regionArray[parentRegion].actualSubSize
							- round(
									(regionArray[childRegion].boundary[3]
											- regionArray[parentRegion].boundary[3])
			...
			|| fabs(
					(regionArray[childRegion].boundary[5]
							- regionArray[parentRegion].boundary[5])
							/ regionArray[parentRegion].actualSubSize
							- round(
									(regionArray[childRegion].boundary[5]
											- regionArray[parentRegion].boundary[5])
...
void lockPointToRegion(double point[3], const short startRegion,
		const short endRegion, const struct region regionArray[],
		const short faceID) {
	short boundRegion;
	short lockInd;
	double coorSq[3];
	...
	case CYLINDER:
		...
		if (regionArray[boundRegion].boundary[4] == PLANE_XY) {
			across1 = 0;
			across2 = 1;
			along = 2;
		} else if (regionArray[boundRegion].boundary[4] == PLANE_XZ) {
			across1 = 0;
			along = 1;
			across2 = 2;
		} else if (regionArray[boundRegion].boundary[4] == PLANE_YZ) {
			along = 0;
			across1 = 1;
			across2 = 2;
		} else {
			fprintf(stderr,
					"ERROR: Cannot determine the orientation of a Cylinder for locking a Molecule.\n");
		}
	...
	default:
		fprintf(stderr,
				"ERROR: Point cannot be locked to region %u (label: \"%s\") because it is of type %d.\n",
				boundRegion, regionArray[boundRegion].spec.label,
...
void generatePointInRegion(const short curRegion,
		const struct region regionArray[], double point[3]) {
...
bool bSubFaceRegion(struct region regionArray[], const short neighRegion,
		const double curSubBound[6], double boundAdjError,
		unsigned short * numFace, unsigned short dirArray[6]) {
	double neighPoint[3];
...
void initializeRegionNesting(const short NUM_REGIONS,
		struct region regionArray[], bool * bFail) {
	short i, j, curChild;
	...
	for (i = 0; i < NUM_REGIONS; i++) { // Assign parents and count number of children in each region
		regionArray[i].bParent = false;
		regionArray[i].parentID = SHRT_MAX;
		curChildArray[i] = 0;
		if (regionArray[i].spec.parent
				&& strlen(regionArray[i].spec.parent) > 0) { // Region has a parent. Determine index of parent
			for (j = 0; j < NUM_REGIONS; j++) {
				if (!strcmp(regionArray[i].spec.parent,
						regionArray[j].spec.label) && i != j) {
			...
			if (!regionArray[i].bParent) {
				...
				fprintf(stderr,
						"ERROR: Region %u (Label: \"%s\") has non-existent parent \"%s\".\n",
						i, regionArray[i].spec.label,
						regionArray[j].spec.parent);
				*bFail = true;
			}
		}
	}
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		if (regionArray[i].numChildren > 0) {
			...
			regionArray[i].childrenID = malloc(
					regionArray[i].numChildren * sizeof(short));
			regionArray[i].childrenCoor = malloc(
					regionArray[i].numChildren * sizeof(uint32_t[6]));
			if (regionArray[i].childrenID == NULL
					|| regionArray[i].childrenCoor == NULL) {
				fprintf(stderr,
						"ERROR: Memory allocation to region %u (label: \"%s\") parameters.\n",
						i, regionArray[i].spec.label);
				exit(EXIT_FAILURE);
			}
		}
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		for (curChild = 0; curChild < regionArray[i].numChildren; curChild++) {
			...
			if (regionArray[i].spec.shape == RECTANGULAR_BOX
				...
				if (bChildRegionNotFlush(i, j, regionArray,
						regionArray[i].subResolution)) {
					// Child is not flush with region's subvolumes
					fprintf(stderr,
							"ERROR: Nested region %u (label: \"%s\") is not properly aligned within parent region %u (label: \"%s\").\n",
							j, regionArray[j].spec.label, i,
							regionArray[i].spec.label);
					fprintf(stderr,
							"Both regions are rectangular and the outer boundary of the nested region must be flush with subvolumes of the parent region.\n");
					*bFail = true;
					continue;
				}
				...
				if (regionArray[j].spec.surfaceType == SURFACE_INNER
						&& (regionArray[i].spec.surfaceType != SURFACE_OUTER
								|| !bBoundarySurround(RECTANGULAR_BOX,
										regionArray[i].boundary,
										RECTANGULAR_BOX,
										regionArray[j].boundary,
										-regionArray[i].subResolution))) {
					fprintf(stderr,
							"ERROR: Nested region %u (label: \"%s\") is an inner-pointing surface with parent region %u (label: \"%s\").\n",
							j, regionArray[j].spec.label, i,
							regionArray[i].spec.label);
					fprintf(stderr,
							"A 3D child can only be an inner surface if its parent is an outer surface.\n");
					*bFail = true;
					continue;
				}
				...
				if (!bBoundarySurround(RECTANGULAR_BOX, regionArray[j].boundary,
						RECTANGULAR_BOX, regionArray[i].boundary,
						-regionArray[i].subResolution)) {
					// Child is sticking out of parent
					fprintf(stderr,
							"ERROR: Outer boundary of nested region %u (label: \"%s\") is not properly surrounded by parent region %u (label: \"%s\").\n",
							j, regionArray[j].spec.label, i,
							regionArray[i].spec.label);
				...
				if (bChildRegionNotFlush(i, j, regionArray,
					...
					fprintf(stderr,
							...
							regionArray[i].spec.label);
					fprintf(stderr,
							"Both regions are rectangles and the outer boundary of the nested region must be flush with subvolumes of the parent region.\n");
					*bFail = true;
				}

				// Confirm that child is inside of parent
				if (regionArray[i].plane != regionArray[j].plane
						|| !bBoundarySurround(RECTANGLE,
								regionArray[j].boundary, RECTANGLE,
								regionArray[i].boundary,
								-regionArray[i].subResolution)) {
					// Child is sticking out of parent
					fprintf(stderr,
				...
				if (regionArray[i].plane == PLANE_XY) {
					...
					&& regionArray[j].spec.shape == RECTANGULAR_BOX) {
				if (!bBoundarySurround(regionArray[j].spec.shape,
						regionArray[j].boundary, SPHERE,
						regionArray[i].boundary,
						regionArray[j].actualSubSize
								- regionArray[i].subResolution)) {
					// Child is sticking out of parent
					fprintf(stderr,
							"ERROR: Outer boundary of nested region %u (label: \"%s\") is not properly surrounded by parent region %u (label: \"%s\").\n",
							...
							regionArray[i].spec.label);
					fprintf(stderr,
							"Rectangular region %u should be inside of spherical region %u and there must be a clearance between the surfaces of at least the subvolume size of region %u which is %.2em.\n",
				...
				if (!regionArray[i].spec.bMicro) {
					...
					fprintf(stderr,
							"ERROR: Normal mesoscopic region %u (label: \"%s\") has a spherical child region %u (label: \"%s\").\n",
							i, regionArray[i].spec.label, j,
							regionArray[j].spec.label);
					*bFail = true;
				}

				if (!bBoundarySurround(SPHERE, regionArray[j].boundary,
						RECTANGULAR_BOX, regionArray[i].boundary,
						regionArray[i].actualSubSize
								- regionArray[i].subResolution)) {
					// Child is sticking out of parent
					fprintf(stderr,
							...
							regionArray[i].spec.label);
					fprintf(stderr,
							"Spherical region %u should be inside of rectangular region %u and there must be a clearance between the surfaces of at least the subvolume size of region %u which is %.2em.\n",
							j, i, i, regionArray[i].actualSubSize);
					*bFail = true;
				}
			} else if (regionArray[i].spec.shape == SPHERE
					&& regionArray[j].spec.shape == SPHERE) {
				if (!bBoundarySurround(SPHERE, regionArray[j].boundary, SPHERE,
						regionArray[i].boundary, 0.)) {
					// Child is sticking out of parent
					fprintf(stderr,
							"ERROR: Outer boundary of spherical nested region %u (label: \"%s\") is not properly surrounded by spherical parent region %u (label: \"%s\").\n",
							j, regionArray[j].spec.label, i,
							regionArray[i].spec.label);
					*bFail = true;
				}
			} else if (regionArray[i].spec.shape == RECTANGULAR_BOX
					&& regionArray[j].spec.shape == RECTANGLE) { // A box region can have a rectangle child only if both regions are surfaces

				// NOTE: Short-circuiting this option for now!
				// Combination of child and parent is invalid
				fprintf(stderr,
						"ERROR: Combination of child/parent for region %u (label: \"%s\") nested in region %u (label: \"%s\") is invalid.\n",
						j, regionArray[j].spec.label, i,
						regionArray[i].spec.label);
				...
				*bFail = true;
				...
				if (regionArray[i].spec.type != REGION_SURFACE_3D
					...
					// Child is sticking out of parent
					fprintf(stderr,
							"ERROR: Region %u (label: \"%s\") has improper child region %u (label: \"%s\").\n",
							i, regionArray[i].spec.label, j,
							regionArray[j].spec.label);
					fprintf(stderr,
							"A rectangle can only be the child of a box region if both regions are surfaces.\n");
					*bFail = true;
				}
				...
				case PLANE_XY:
					bFailRectangleChild =
							!(regionArray[i].childrenCoor[curChild][4]
									|| regionArray[i].childrenCoor[curChild][5]);
					break;
				case PLANE_XZ:
					bFailRectangleChild =
							!(regionArray[i].childrenCoor[curChild][2]
									|| regionArray[i].childrenCoor[curChild][3]);
					break;
				case PLANE_YZ:
					bFailRectangleChild =
							!(regionArray[i].childrenCoor[curChild][0]
									|| regionArray[i].childrenCoor[curChild][1]);
					break;
				}
				if (bFailRectangleChild) {
					fprintf(stderr,
							"ERROR: Nested region %u (label: \"%s\") is not on boundary of parent region %u (label: \"%s\").\n",
							j, regionArray[j].spec.label, i,
							regionArray[i].spec.label);
					fprintf(stderr,
							"A rectangular surface region can only have a parent surface if it is directly on the surface of the parent.\n");
					*bFail = true;
					continue;
				}
		...
		if (regionArray[i].spec.shape == SPHERE)
			...
			if (regionArray[i].plane == PLANE_3D) {
				if (regionArray[i].spec.type == REGION_NORMAL) {
					...
					regionArray[i].numSub = (uint32_t) 2
							* regionArray[i].spec.numX
							* regionArray[i].spec.numY
							+ 2 * regionArray[i].spec.numY
									* regionArray[i].spec.numZ
							+ 2 * regionArray[i].spec.numX
									* regionArray[i].spec.numZ;
				}
			} else {
			...
			for (curChild = 0; curChild < regionArray[i].numChildren;
					curChild++) {
				j = regionArray[i].childrenID[curChild];
				if (regionArray[j].spec.shape == RECTANGULAR_BOX
						&& regionArray[i].spec.type == REGION_NORMAL) { // A 3D child must have a 3D parent, and the subvolumes of
					...
					for (curZ = 0; curZ < regionArray[i].spec.numZ; curZ++) {
						for (curY = 0; curY < regionArray[i].spec.numY;
								curY++) {
							for (curX = 0; curX < regionArray[i].spec.numX;
									curX++) {
								subCoorInd[0] = curX;
								subCoorInd[1] = curY;
								subCoorInd[2] = curZ;
								findSubvolCoor(curSubBound, regionArray[i],
										subCoorInd);
								if (bBoundarySurround(RECTANGULAR_BOX,
										curSubBound, SPHERE,
										regionArray[regionArray[i].childrenID[curChild]].boundary,
										0.)) {
						...
						&& (regionArray[i].spec.shape == RECTANGLE
								|| regionArray[i].spec.type != REGION_NORMAL)) { // Only subtract area of children if parent is 2D
					switch (regionArray[j].plane) {
					case PLANE_XY:
						regionArray[i].numSub -=
								(uint32_t) regionArray[j].spec.numX
					...
					case PLANE_YZ:
						regionArray[i].numSub -=
								(uint32_t) regionArray[j].spec.numY
										* regionArray[j].spec.numZ;
						break;
						break;
					}
...
void allocateRegionNeighbors(const short NUM_REGIONS,
	...
	for (i = 0; i < NUM_REGIONS; i++) {
		...
		regionArray[i].regionNeighDir = malloc(
				NUM_REGIONS * sizeof(unsigned short));
		if (regionArray[i].isRegionNeigh == NULL
				|| regionArray[i].regionNeighDir == NULL) {
			fprintf(stderr,
					"ERROR: Memory allocation for region %u (label: \"%s\") parameters.\n",
					i, regionArray[i].spec.label);
			exit(EXIT_FAILURE);
		}
		...
		if (regionArray[i].spec.bMicro) {
			regionArray[i].numRegionNeighFace = malloc(
					NUM_REGIONS * sizeof(short));
			regionArray[i].boundRegionFaceCoor = malloc(
		...
		regionArray[i].numSubRegionNeigh = malloc(
				NUM_REGIONS * sizeof(uint32_t));
		if (regionArray[i].numSubRegionNeigh == NULL) {
			fprintf(stderr,
					"ERROR: Memory allocation for region %u (label: \"%s\") parameters.\n",
					i, regionArray[i].spec.label);
		...
		for (j = 0; j < NUM_REGIONS; j++) { // Initialize isRegionNeigh to false
...
void validateRegions(const short NUM_REGIONS, const struct region regionArray[],
		bool * bFail) {
	...
	for (curRegion = 0; curRegion < NUM_REGIONS; curRegion++) {
		...
		for (curChild1 = 0; curChild1 < regionArray[curRegion].numChildren;
				curChild1++) { // Compare with every higher-ID child
			...
			for (curChild2 = curChild1 + 1;
					curChild2 < regionArray[curRegion].numChildren;
					curChild2++) {
				curChild2ID = regionArray[curRegion].childrenID[curChild2];

				intersectType = intersectBoundary(
						regionArray[curChild1ID].spec.shape,
						regionArray[curChild1ID].boundary,
						regionArray[curChild2ID].spec.shape,
						regionArray[curChild2ID].boundary, intersection);

				if (intersectType == UNDEFINED_SHAPE
						|| boundaryVolume(intersectType, intersection)
								> minVol) { // Intersection detected
					fprintf(stderr,
							"ERROR: Nested region %u (label: \"%s\") overlaps nested region %u (label: \"%s\"). Both regions are children of region %u (label: \"%s\").\n",
							curChild1ID, regionArray[curChild1ID].spec.label,
							curChild2ID, regionArray[curChild2ID].spec.label,
							curRegion, regionArray[curRegion].spec.label);
					*bFail = true;
				}
			}
		...
		if (!regionArray[curRegion].bParent) {
			for (otherRegion = curRegion + 1; otherRegion < NUM_REGIONS;
					otherRegion++) {
				...
				intersectType = intersectBoundary(
						regionArray[curRegion].spec.shape,
						regionArray[curRegion].boundary,
						regionArray[otherRegion].spec.shape,
				...
				if ((regionArray[curRegion].spec.shape == RECTANGLE
					...
					if (boundaryVolume(RECTANGLE, intersection)
							> squareDBL(
									regionArray[squareRegion].subResolution)) { // Intersection is possible.
						...
						bTemp =
								!(fabs(
										regionArray[squareRegion].boundary[dim]
												- regionArray[boxRegion].boundary[dim])
										< regionArray[squareRegion].subResolution
										|| fabs(
												regionArray[squareRegion].boundary[dim]
														- regionArray[boxRegion].boundary[dim
																+ 1])
				...
				if (bTemp) { // Intersection detected
					fprintf(stderr,
							"ERROR: Region %u (label: \"%s\") overlaps region %u (label: \"%s\"). Both regions have no parent.\n",
							curRegion, regionArray[curRegion].spec.label,
							otherRegion, regionArray[otherRegion].spec.label);
					*bFail = true;
				}
			}
		} else if (regionArray[curRegion].spec.shape == RECTANGLE) { // Region is rectangle and has a parent
			otherRegion = regionArray[curRegion].parentID;
			if (regionArray[otherRegion].plane == PLANE_3D
					&& !regionArray[otherRegion].spec.bMicro) { // Parent is 3D and mesoscopic.
				...
				bTemp = false;
				for (curNeigh = 0;
						curNeigh < regionArray[curRegion].numRegionNeigh;
						curNeigh++) {
					neighID = regionArray[curRegion].regionNeighID[curNeigh];
					if (neighID
							== otherRegion|| regionArray[neighID].plane != PLANE_3D)continue; // Don't check against parent or 2D neighbor

					if (!bTemp) {
						bTemp = true;
						initDir =
								regionArray[curRegion].regionNeighDir[curNeigh];
					} else if (initDir
							!= regionArray[curRegion].regionNeighDir[curNeigh]) {
						bDoubleSide = true;
					}
					...
					intersectBoundary(regionArray[curRegion].spec.shape,
							regionArray[curRegion].boundary,
							regionArray[neighID].spec.shape,
				...
				if ((bDoubleSide
						&& fabs(curVol - 2 * regionArray[curRegion].volume)
								> minVol)
						|| fabs(curVol - regionArray[curRegion].volume)
								> minVol) {
					fprintf(stderr,
							"ERROR: Region %u (label: \"%s\") is a rectangle child of mesoscopic 3D region %u (label: \"%s\") but is not on boundary of region %u's children.\n",
							curRegion, regionArray[curRegion].spec.label,
							otherRegion, regionArray[otherRegion].spec.label,
							otherRegion);
					*bFail = true;
...
void checkSurfaceRegionChildren(const short curRegion, const short NUM_REGIONS,
		const struct region regionArray[], bool * bFail) {
	double curArea = 0.;
	...
	case SPHERE:
		...
		// child is valid
		if (regionArray[curRegion].numChildren
				> 1|| regionArray[curRegion].boundary[3] != regionArray[regionArray[curRegion].childrenID[0]].boundary[3]
				|| regionArray[regionArray[curRegion].childrenID[0]].spec.shape != SPHERE) {fprintf(stderr, "ERROR: Surface spherical region %u (label: \"%s\") has more than 1 child, or its child region %u (label: \"%s\") is not a sphere with the same radius.\n",
...
bool bSurfaceBetweenBoundaries(const struct region regionArray[],
		const short NUM_REGIONS, const short region1, const short region2,
		const double boundary1[6], const double boundary2[6],
	...
	for (*surfaceRegion = 0; *surfaceRegion < NUM_REGIONS; (*surfaceRegion)++) {
		if (regionArray[*surfaceRegion].spec.type != REGION_NORMAL
				&& regionArray[*surfaceRegion].spec.surfaceType
						!= SURFACE_MEMBRANE
				&& ((regionArray[region1].isRegionNeigh[*surfaceRegion]
						&& regionArray[region1].regionNeighDir[*surfaceRegion]
								!= CHILD)
						|| (regionArray[region2].isRegionNeigh[*surfaceRegion]
								&& regionArray[region2].regionNeighDir[*surfaceRegion]
										!= CHILD))) { // surfaceRegion is a "one-sided" surface.
			...
			if (bLineHitBoundary(p1, trajLine, lineLength,
					...
					regionArray[*surfaceRegion].boundary, NULL,
					regionArray[*surfaceRegion].plane, false, &d, p2)) { // Subvolumes are not true neighbors
				return false;
			}
		}
	}
...
// fully contains or is contained by the other boundary
// Round children must be entirely inside or outside intersection
bool bIntersectRegion(const short curRegion, const struct region regionArray[],
