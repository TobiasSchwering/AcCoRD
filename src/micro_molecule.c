...
static void traverseRecent(const ListMolRecent3D * p_list,
		void (*p_fun)(ItemMolRecent3D item));
...
static void removeItemRecent(NodeMolRecent3D * prevNode,
		NodeMolRecent3D * curNode);
...
// Move one molecule to the specified coordinates
void moveMoleculeRecent(ItemMolRecent3D * molecule, double x, double y,
		double z) {
	molecule->x = x;
	molecule->y = y;
	molecule->z = z;
	...
	for (curRegion = 0; curRegion < NUM_REGIONS; curRegion++) {
		for (curType = 0; curType < NUM_MOL_TYPES; curType++) {
			...
			while (curNode != NULL) {
				...
				if (curNode->item.bNeedUpdate) { // Molecule needs to be moved
					...
					if (validateMolecule(newPoint, oldPoint, NUM_REGIONS,
						...
						if (newRegion == curRegion) { //just update molecule location
							moveMolecule(&curNode->item, newPoint[0],
									newPoint[1], newPoint[2]);
							prevNode = curNode;
						} else {
							// Molecule is now in a different region
							if (regionArray[newRegion].spec.bMicro) { // New region is microscopic. Move to appropriate list

								if (bReaction) { // We need to fire the corresponding reaction curRxn
									if (regionArray[newRegion].numRxnProducts[curRxn]
											> 0) {
										for (curProd = 0;
												curProd
														< regionArray[newRegion].numRxnProducts[curRxn];
												curProd++) {
											...
											if (!addMolecule(
													...
													newPoint[2])) { // Creation of molecule failed
												fprintf(stderr,
														"ERROR: Memory allocation to create molecule of type %u from reaction %u.\n",
														regionArray[newRegion].productID[curRxn][curProd],
														curRxn);
												exit(EXIT_FAILURE);
											}
											// Indicate that product molecule doesn't need to be
											// moved again
											p_list[newRegion][regionArray[newRegion].productID[curRxn][curProd]]->item.bNeedUpdate =
													false;
										}
									}
								} else if (!addMolecule(
										&p_list[newRegion][curType],
										newPoint[0], newPoint[1],
										newPoint[2])) {
									fprintf(stderr,
											"ERROR: Memory allocation to move molecule between microscopic regions %u and %u.\n",
											curRegion, newRegion);
									exit(EXIT_FAILURE);
								} else {
									// Indicate that molecule doesn't need to be moved again
									p_list[newRegion][curType]->item.bNeedUpdate =
											false;
								}
							} else { // New region is mesoscopic. Find nearest subvolume to new point
								newSub = findNearestSub(newRegion, regionArray,
										transRegion, newPoint[0], newPoint[1],
										newPoint[2]);
								...
								regionArray[newRegion].bNeedUpdate[transRegion][curBoundSub] =
										true;
							...
							if (prevNode == NULL) { // We removed first molecule in list.
	...
	for (curRegion = 0; curRegion < NUM_REGIONS; curRegion++) {
		for (curType = 0; curType < NUM_MOL_TYPES; curType++) {
			...
			while (curNodeR != NULL) {
				...
				// Diffuse molecule
				diffuseOneMoleculeRecent(&curNodeR->item,
						DIFF_COEF[curRegion][curType]);
				...
				validateMolecule(newPoint, oldPoint, NUM_REGIONS, curRegion,
						&newRegion, &transRegion, regionArray, curType,
						&bReaction, &curRxn);

				if (regionArray[newRegion].spec.bMicro) { // Region is microscopic. Move to appropriate list
					if (bReaction) { // We need to fire the corresponding reaction curRxn
						...
						fprintf(stderr,
								"ERROR: Memory allocation to move molecule between recent molecule list of region %u and list of region %u.\n",
								curRegion, newRegion);
						exit(EXIT_FAILURE);
					}
				} else { // New region is mesoscopic. Find nearest subvolume to new point
					newSub = findNearestSub(newRegion, regionArray, transRegion,
							newPoint[0], newPoint[1], newPoint[2]);
					subvolArray[newSub].num_mol[curType]++;
					// TODO: Keep track of subvolumes that need updated propensities
					// mesoID is subvolArray[newSub].mesoID
...
void rxnFirstOrder(ListMol3D * p_list, const struct region regionArray,
		unsigned short curMolType, const unsigned short NUM_MOL_TYPES,
	...
	while (curNode != NULL) {
		...
		for (i = 0; i < regionArray.numFirstCurReactant[curMolType]; i++) {
			if (curRand < regionArray.uniCumProb[curMolType][i]) {
				...
				if (regionArray.numRxnProducts[curRxn] > 0) {
					...
					// were generated
					curTime = -log(
							(1. - regionArray.minRxnTimeRV[curMolType])
									* mt_drand()
					...
					for (curProd = 0;
						...
						if (!addMoleculeRecent(
								...
								curNode->item.x, curNode->item.y,
								curNode->item.z,
								regionArray.spec.dt - curTime)) { // Creation of molecule failed
							fprintf(stderr,
									"ERROR: Memory allocation to create molecule of type %u from reaction %u.\n",
									regionArray.productID[curRxn][curProd],
									curRxn);
							exit(EXIT_FAILURE);
						}
					}
		...
		if (prevNode == NULL && bRemove) { // prevNode does not change, but we removed first molecule in list.
										   // nextNode is now the start of the list
										   // (i.e., we must update pointer to list)
			*p_list = nextNode;
		} else if (!bRemove) {
...
void rxnFirstOrderRecent(const unsigned short NUM_MOL_TYPES,
		ListMolRecent3D pRecentList[NUM_MOL_TYPES],
		const struct region regionArray, unsigned short curMolType,
		bool bCheckCount, uint32_t numMolCheck[NUM_MOL_TYPES]) {
	...
	while (curNode != NULL) {
		if (bCheckCount && ++curMol > numMolOrig) { // We only need to check the first numMolOrig molecules in this list
				...
				if (regionArray.numRxnProducts[curRxn] > 0) {
					...
					minRxnTimeRV = exp(
							-curNode->item.dt_partial
									* regionArray.uniSumRate[curMolType]);
					curTime = -log(
							(1. - minRxnTimeRV) * mt_drand() + minRxnTimeRV)
							/ regionArray.uniSumRate[curMolType];
					for (curProd = 0;
							curProd < regionArray.numRxnProducts[curRxn];
							curProd++) {
						...
						// Add the (curProd)th product to the corresponding molecule list
						if (!addMoleculeRecent(&pRecentList[prodID],
								curNode->item.x, curNode->item.y,
								curNode->item.z,
								curNode->item.dt_partial - curTime)) { // Creation of molecule failed
							fprintf(stderr,
									"ERROR: Memory allocation to create molecule of type %u from reaction %u.\n",
									regionArray.productID[curRxn][curProd],
									curRxn);
						...
						if (prevNode == NULL && prodID == curMolType) { // We've just added a molecule to the start of the current
	...
	if (bCheckCount) { // Correct the number of molecules to check in next round
...
void transferMolecules(ListMolRecent3D * molListRecent, ListMol3D * molList) {
	...
	while (p_node != NULL) {
		if (!addMolecule(molList, p_node->item.x, p_node->item.y,
				p_node->item.z)) {
			// Creation of molecule failed
			fprintf(stderr,
					"ERROR: Memory allocation to create molecule when transferring from recent list to regular list.\n");
			exit(EXIT_FAILURE);
		}
		p_node = p_node->next;
	}
...
bool validateMolecule(double newPoint[3], double oldPoint[3],
		const short NUM_REGIONS, const short curRegion, short * newRegion,
		short * transRegion, const struct region regionArray[], short molType,
		bool * bReaction, unsigned short * curRxn) {
	double trajLine[3];
	double lineLength;
	...
	*newRegion = curRegion;
	...
	if (regionArray[curRegion].numChildren < 1
			&& bPointInRegionNotChild(curRegion, regionArray, newPoint)) { // This is simplest case. No region boundary interactions
		return true;
	} else { // Molecule may have left region's outer boundary or went through a child region

		// Define trajectory vector
		defineLine(oldPoint, newPoint, trajLine, &lineLength);

		return followMolecule(oldPoint, newPoint, trajLine, lineLength,
				curRegion, newRegion, transRegion, regionArray, molType,
				bReaction, curRxn, 0);
	}
}
...
bool followMolecule(const double startPoint[3], double endPoint[3],
		double lineVector[3], double lineLength, const short startRegion,
		short * endRegion, short * transRegion,
	...
	*endRegion = SHRT_MAX;
	...
	for (curNeigh = 0; curNeigh < regionArray[startRegion].numRegionNeigh;
		...
		case CHILD:
			...
			bCurIntersect = bLineHitRegion(startPoint, lineVector, lineLength,
					startRegion, curRegion, regionArray, &curFace, &curDist,
					curIntersectPoint);
			break;
		default:
		...
		if (bCurIntersect
				&& curDist < minDist + regionArray[startRegion].subResolution) { // There is intersection with this face and it is ~closest so far
			...
			if (*endRegion < SHRT_MAX
					...
					&& minDist - curDist
							< regionArray[startRegion].subResolution) { // curRegion is not effectively any closer than a surface region, so do not override
				continue;
			}
			minDist = curDist;
			*endRegion = curRegion;
	...
	if (*endRegion < SHRT_MAX) {
		...
		lockPointToRegion(nearestIntersectPoint, startRegion, *endRegion,
				regionArray, nearestFace);

		if (regionArray[*endRegion].spec.type != REGION_NORMAL) { // Closest region is some kind of surface
																  // Check whether we are actually in child of surface
			if (regionArray[*endRegion].spec.shape != SPHERE
					&& !bPointInRegionOrChild(*endRegion, regionArray,
							nearestIntersectPoint, endRegion, true)) { // Something went wrong here because we are not actually
																	   // in the endRegion region (or its children)
				fprintf(stderr,
						"ERROR: Invalid transition from region %u (Label: \"%s\") to region %u (Label: \"%s\"). Leaving molecule in \"%s\"\n",
			...
			if (regionArray[*endRegion].numFirstRxn > 0) { // There is at least 1 possible reaction
				curRand = mt_drand();
				for (i = 0;
						i < regionArray[*endRegion].numFirstCurReactant[molType];
						i++) {
					if (curRand
							< regionArray[*endRegion].uniCumProb[molType][i]) {
						// Reaction i took place
						*curRxn =
								regionArray[*endRegion].firstRxnID[molType][i];
						*bReaction = true;
						continue;
					}
				}
			...
			if (*bReaction) {
				...
				case SURFACE_MEMBRANE:
					...
					if (minNormalDist
							> minDist
									+ regionArray[startRegion].subResolution) { // Normal region at same distance as membrane was not detected
																				// Check neighbors of membrane for closest normal region
																				// Scan normal neighbors of membrane that are children or parent
						for (curNeigh = 0;
								curNeigh
										< regionArray[*endRegion].numRegionNeigh;
								curNeigh++) {
							curRegion =
									regionArray[*endRegion].regionNeighID[curNeigh];

							if (curRegion
									== startRegion|| regionArray[curRegion].spec.type != REGION_NORMAL)continue;
							...
							case CHILD:
								...
								// Need to check against all faces of child
								bCurIntersect = bLineHitRegion(startPoint,
										lineVector, lineLength, *endRegion,
										curRegion, regionArray, &curFace,
										&curDist, curIntersectPoint);
								break;
							default:
								// Regions are adjacent. Only check adjacent plane
								curFace =
										regionArray[*endRegion].regionNeighDir[curRegion];
								bCurIntersect =
										bLineHitInfinitePlane(startPoint,
												lineVector, lineLength,
												RECTANGULAR_BOX,
												regionArray[*endRegion].boundRegionFaceCoor[curRegion][0],
												curFace, false, &curDist,
												curIntersectPoint)
												&& bPointOnFace(
														curIntersectPoint,
														RECTANGULAR_BOX,
							...
							if (bCurIntersect && curDist < minNormalDist) { // There is intersection with this face and it is closest normal region so far
								minNormalDist = curDist;
								closestNormal = curRegion;
								nearestFace = curFace;
								nearestIntersectPoint[0] = curIntersectPoint[0];
						...
						if (minNormalDist
								> minDist
										+ regionArray[startRegion].subResolution) { // Could not find region in membrane neighbor list
																					// Region is probably a grandparent or grandchild to membrane
							fprintf(stderr,
									"ERROR: Could not evaluate transition from region %u (Label: \"%s\") through membrane region %u (Label: \"%s\").\n",
									startRegion,
									regionArray[startRegion].spec.label,
									*endRegion,
									regionArray[*endRegion].spec.label);
							exit(EXIT_FAILURE);
						}
					}
		...
		if (bReflect) {
			...
			*endRegion = startRegion;
			...
			if (bContinue) {
				...
				while (!bPointInBoundary(curIntersectPoint,
				...
				if (!bPointInRegionOrChild(*endRegion, regionArray,
						curIntersectPoint, endRegion, false)) { // Something went wrong here because we are not actually
																// in the endRegion region (or its children)
					fprintf(stderr,
							"ERROR: Invalid transition from region %u (Label: \"%s\") to region %u (Label: \"%s\"). Leaving molecule in \"%s\"\n",
							startRegion, regionArray[startRegion].spec.label,
				...
				return followMolecule(curIntersectPoint, endPoint, lineVector,
						...
						regionArray, molType, bReaction, curRxn, depth + 1)
	...
	if (!reflectPoint(startPoint, lineVector, lineLength, endPoint, newEndPoint,
			nearestIntersectPoint, &nearestFace,
			regionArray[reflectRegion].spec.shape,
			regionArray[reflectRegion].boundary, bReflectInside)) { // Reflection failed. Leave point at nearestIntersectPoint
		*endRegion = startRegion;
		endPoint[0] = nearestIntersectPoint[0];
		endPoint[1] = nearestIntersectPoint[1];
		endPoint[2] = nearestIntersectPoint[2];
	...
	lockPointToRegion(nearestIntersectPoint, startRegion, startRegion,
			regionArray, nearestFace);
	...
	followMolecule(nearestIntersectPoint, newEndPoint, lineVector, lineLength,
			startRegion, endRegion, transRegion, regionArray, molType,
...
uint64_t recordMolecules(ListMol3D * p_list, ListMol3D * recordList,
		int obsType, double boundary[], bool bRecordPos, bool bRecordAll) {
	NodeMol3D * p_node = *p_list;
	uint64_t curCount = 0ULL;

	while (p_node != NULL) {
		if (bRecordAll
				|| isMoleculeObserved(&p_node->item, obsType, boundary)) {
			curCount++;
...
uint64_t recordMoleculesRecent(ListMolRecent3D * p_list, ListMol3D * recordList,
	...
	while (p_node != NULL) {
		if (bRecordAll
				|| isMoleculeObservedRecent(&p_node->item, obsType, boundary)) {
			curCount++;
			if (bRecordPos
					&& !addMolecule(recordList, p_node->item.x, p_node->item.y,
							p_node->item.z)) {
				fprintf(stderr,
						"\nERROR: Memory allocation for recording molecule positions.\n");
				exit(EXIT_FAILURE);
...
bool isMoleculeObserved(ItemMol3D * molecule, int obsType, double boundary[]) {
	...
	case CYLINDER:
		if (boundary[4] == PLANE_XY) {
			return squareDBL(molecule->x - boundary[0])
					+ squareDBL(molecule->y - boundary[1])
					<= squareDBL(boundary[3]) && molecule->z >= boundary[2]
					&& molecule->z <= boundary[2] + boundary[5];
		} else if (boundary[4] == PLANE_XZ) {
			return squareDBL(molecule->x - boundary[0])
					+ squareDBL(molecule->z - boundary[2])
					<= squareDBL(boundary[3]) && molecule->y >= boundary[1]
					&& molecule->y <= boundary[1] + boundary[5];
		} else if (boundary[4] == PLANE_YZ) {
			return squareDBL(molecule->y - boundary[1])
					+ squareDBL(molecule->z - boundary[2])
					<= squareDBL(boundary[3]) && molecule->x >= boundary[0]
					&& molecule->x <= boundary[0] + boundary[5];
		} else {
			fprintf(stderr,
					"ERROR: Cannot determine the orientation of a Cylinder.\n");
			return false;
		}
	default:
		fprintf(stderr,
				"ERROR: Cannot determine whether a molecule is inside a boundary for observation.\n");
		return false;
	}
}
...
// Visit each node and execute the function pointed to by p_fun
// p_fun must be a void function that takes an item as input
void traverseRecent(const ListMolRecent3D * p_list,
		void (*p_fun)(ItemMolRecent3D item)) {
	NodeMolRecent3D * p_node = *p_list;
