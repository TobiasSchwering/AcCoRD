...
// Do two sets of boundaries overlap? TODO add cylinder support
...
// Both boundaries must be rectangular (either 2D or 3D) TODO add cylinders face-on-face
                      ...
                      double * d, double intersectPoint[3])
{
    ...
    case RECTANGULAR_BOX:
        ...
        return false;
        //TODO: integrate bLineHitInfinitePlane and bPointOnFace
        for (curPlane = 0; curPlane < 6; curPlane++)
        {
            ...
            {
                if (planeID == 4)
                    *d = (boundary1[2] - p1[2]) / L[2];
                else if (planeID == 5)
                    *d = (boundary1[2] + boundary1[5] - p1[2]) / L[2];
                else   //TODO calculation stolen from SPHERE and reduced by a dimension. still correct?
                {
                    ...
                    *d = sqrt(
                             ...
                             - squareDBL(centerToP1[0])
                             - squareDBL(centerToP1[1]));
                    if (bInside)
                        *d = -LDotCenterToP1 + *d;
                    else
                        *d = -LDotCenterToP1 - *d;
            ...
            {
                if (planeID == 2)
                    *d = (boundary1[1] - p1[1]) / L[1];
                else if (planeID == 3)
                    *d = (boundary1[1] + boundary1[5] - p1[1]) / L[1];
                else   //TODO calculation stolen from SPHERE and reduced by a dimension. still correct?
                {
                    centerToP1[0] = p1[0] - boundary1[0];
                    centerToP1[2] = p1[2] - boundary1[2];
                    LDotCenterToP1 = L[0] * centerToP1[0] + L[2] * centerToP1[2];
                    *d = sqrt(
                             squareDBL(LDotCenterToP1) + squareDBL(boundary1[3])
                             - squareDBL(centerToP1[0])
            ...
            else if (boundary1[4] == PLANE_YZ)
            {
                if (planeID == 0)
                    ...
                    *d = (boundary1[0] + boundary1[5] - p1[0]) / L[0];
                else   //TODO calculation stolen from SPHERE and reduced by a dimension. still correct?
                {
                    centerToP1[1] = p1[1] - boundary1[1];
                    centerToP1[2] = p1[2] - boundary1[2];
                    LDotCenterToP1 = L[1] * centerToP1[1] + L[2] * centerToP1[2];
                    *d = sqrt(
                             squareDBL(LDotCenterToP1) + squareDBL(boundary1[3])
                             - squareDBL(centerToP1[1])
                             - squareDBL(centerToP1[2]));
            ...
            if (bLineHitInfinitePlane(p1, L, length, CYLINDER, boundary1,
                                      curPlane, bInside, d, intersectPoint)
                    && bPointOnFace(intersectPoint, CYLINDER, boundary1,
                                    curPlane) && *d < minDist)   // Line does intersect this face at a valid distance and it is closest
            {
                bIntersect = true;
        ...
        if (boundary1[4] == PLANE_XY)
        {
            if (planeID == 4 || planeID == 5)
                ...
                return (sqrt(
                            ...
                            + squareDBL(p1[1] - boundary1[1]))
                        == boundary1[5]
                        && (boundary1[2] <= p1[2] && p1[2] <= boundary1[2] + boundary1[5]));
        }
        else if (boundary1[4] == PLANE_XZ)
        {
            if (planeID == 2 || planeID == 3)
                ...
                return (sqrt(
                            ...
                            + squareDBL(p1[2] - boundary1[2]))
                        == boundary1[5]
                        && (boundary1[1] <= p1[1] && p1[2] <= boundary1[1] + boundary1[5]));
        }
        else if (boundary1[4] == PLANE_YZ)
        {
            if (planeID == 0 || planeID == 1)
                ...
                return (sqrt(
                            ...
                            + squareDBL(p1[2] - boundary1[2]))
                        == boundary1[5]
                        && (boundary1[0] <= p1[0] && p1[2] <= boundary1[0] + boundary1[5]));
        }
        else
        {
            fprintf(stderr,
                  ...
                  const double boundary1[], bool bReflectInside)
{
    ...
    default:
        fprintf(stderr, "ERROR: Cannot reflect a point off of an unknown region.\n");
        return false;
    }
}
